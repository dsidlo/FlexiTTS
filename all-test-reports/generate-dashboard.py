...
TEST_REPORT_DEFS = [
    # (name, description, path relative to repo root)
...
def _coverage_pct_for(rel_path: str) -> Optional[str]:
    """Overall coverage percentage for a coverage report page:
    - python coverage: pc_cov in the generated html
    - UI coverage: statements pct from the istanbul index
    ...
    try:
        c = p.read_text()
        m = re.search(r'pc_cov">(\d+)%', c)
        if m:
            return m.group(1) + "%"
        m = re.search(r'class="strong">([\d.]+)%', c)
        if m:
            return m.group(1) + "%"
    except Exception:
        pass
...
def _report_rows(defs, show_errors: bool) -> str:
    ...
    show_errors: False for coverage reports (error counts don't apply)."""
    rows = []
    for name, desc, rel_path in defs:
        ...
        if show_errors:
            ...
            pct = _coverage_pct_for(rel_path)
            err_cell = (f'<span style="color: #22c55e; font-weight: 600;">{pct}</span>'
                        if pct else '&nbsp;')
        rows.append(
...
<div class="container">
  ...
  <div class="section">
    ...
    <p style="color: #9ca3af; font-size: 0.9em; margin-bottom: 12px;">
      Test report pages, with what each contains and where it lives. All paths
      are clickable.
    </p>
...
{_report_rows(TEST_REPORT_DEFS, show_errors=True)}
    </table>
  </div>
  <div class="section">
    <h2>📊 Coverage Reports</h2>
    <p style="color: #9ca3af; font-size: 0.9em; margin-bottom: 12px;">
      Coverage reports are regenerated with each full test run.
    </p>
    <table style="width: 100%; border-collapse: collapse;">
      <tr style="border-bottom: 2px solid #1d4ed8;">
        <th style="padding: 10px; text-align: left; color: #4fc3f7;">Report</th>
        <th style="padding: 10px; text-align: left; color: #4fc3f7;">What it shows</th>
        <th style="padding: 10px; text-align: center; color: #4fc3f7;">Coverage %</th>
        <th style="padding: 10px; text-align: left; color: #4fc3f7;">Generated</th>
        <th style="padding: 10px; text-align: left; color: #4fc3f7;">Location (click to open)</th>
      </tr>
{_report_rows(COVERAGE_REPORT_DEFS, show_errors=False)}
    </table>
  </div>
  <footer class="footer">
