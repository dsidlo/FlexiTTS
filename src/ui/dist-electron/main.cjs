...
electron_1.ipcMain.handle('run-python-script', async (event, scriptPath, args) => {
    return new Promise((resolve, reject) => {
        ...
        const pythonProcess = (0, child_process_1.spawn)('uv', ['run', 'python', scriptPath, ...args], {
...
electron_1.ipcMain.handle('kill-process', async (event, matchString) => {
...
electron_1.ipcMain.handle('check-xml-exists', async (event, chapterStem) => {
    const projectRoot = path.resolve(__dirname, '../../../');
...
electron_1.ipcMain.handle('show-error-dialog', async (event, title, message) => {
...
electron_1.ipcMain.handle('check-chapter-audio', async (event, chapterName) => {
    const projectRoot = path.resolve(__dirname, '../../../');
...
// Play audio file
electron_1.ipcMain.handle('play-sound-file', async (event, filePath) => {
    return new Promise((resolve, reject) => {
        ...
        const playProcess = (0, child_process_1.spawn)(cmd, args);
